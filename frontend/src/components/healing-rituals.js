  ...
  const getRitualContent = (ritual) => {
    ...
    return (
      <div className="space-y-6">
        ...
        {/* Only show these if no videos were found OR if the rituals are 'release' or 'gratitude' which don't inherently require videos */}
        {!selectedVideo && (
          (videos.length === 0 && !isVideoLoading && !videoError) ||
          ritual.id === "release" ||
          ritual.id === "gratitude"
        ) && (
          <>
            ...
            {(ritual.id === "breathing" || ritual.id === "meditation" || ritual.id === "energy") &&
             videos.length === 0 && !isVideoLoading && !videoError && (
              <div className="text-center space-y-6">
                ...
                <p className="text-white/90">No guided sessions found. You can still continue the ritual without a video.</p>
                <Button
                  onClick={completeRitual}
                  className="bg-white/20 hover:bg-white/30 text-white border border-white/30"
                >
